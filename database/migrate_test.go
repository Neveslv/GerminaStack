...
func TestMigrateExecutesCoreSchemaBeforeTwoFactorChallengeSchema(t *testing.T) {
	...
	discussionFunctionsPattern := regexp.QuoteMeta("CREATE OR REPLACE FUNCTION reaction(")
	...
	mock.ExpectExec(discussionFunctionsPattern).WillReturnResult(sqlmock.NewResult(0, 0))
