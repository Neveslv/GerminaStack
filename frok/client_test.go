...
func TestClientReplyUsesGroqResponsesWithBrowserSearch(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if err != nil || !strings.Contains(string(body), `"model":"openai/gpt-oss-20b"`) || !strings.Contains(string(body), `"type":"browser_search"`) || !strings.Contains(string(body), "provocadora") || !strings.Contains(string(body), "texto cru") {
