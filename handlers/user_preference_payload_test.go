...
func TestUpdateAccessibilityPreferencesForwardsOnlyValidatedPreferencePayload(t *testing.T) {
	...
	recorder := performUserAccountRequest(NewUserHandler(repo, time.Second).UpdateAccessibilityPreferences, http.MethodPut, "/api/users/me/preferences", `{"contrast_theme":"dark","font_family":"arial","font_spacing":"grande","font_size":"grande"}`, true)
