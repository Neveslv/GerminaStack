...
func TestRegisterUserAccountRoutesRegistersOnlyAuthenticatedAccountEndpoints(t *testing.T) {
	...
	for _, want := range []string{"GET /api/users/me", "PUT /api/users/me/preferences"} {
		if !routes[want] {
	...
	for _, tt := range []struct{ method, path string }{{http.MethodGet, "/api/users/me"}, {http.MethodPut, "/api/users/me/preferences"}} {
